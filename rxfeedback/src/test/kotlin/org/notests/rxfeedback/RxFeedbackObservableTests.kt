...
class RxFeedbackObservableTests {
    ...
    fun testImmediateFeedbackLoop() {
        val res = scheduler.start {
            val feedbackLoop: Feedback<String, String> = { observableSchedulerContext ->
                observableSchedulerContext.source.flatMap {
                    when (it) {
    ...
    fun testImmediateFeedbackLoopParallel_react_set() {
        ...
        assertTrue(
            (res.events()[1].value as Event.Next<String>).value.contains("_a") ||
        ...
        assertEquals(
            listOf("initial", "initial_x", "initial_x_x", "initial_x_x_c", "initial_x_x_c_c", "initial_x_x_c_c_c"),
            ignoringAB
        )
    }
    ...
    fun testUIBindFeedbackLoopReentrancy() {
        val res = scheduler.start {
            Observables.system(
                ...
                scheduler = scheduler,
                scheduledFeedback = listOf(bind { observableSchedulerContext ->
                    val results = observableSchedulerContext.source.flatMap {
                        when (it) {
                            "initial" -> Observable.just("_a")
                            "initial_a" -> Observable.just("_b")
                            "initial_a_b" -> Observable.just("_c")
